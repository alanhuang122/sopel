...
def search(word, id, key):
    data = requests.get(url + '/search/en', params = {'q': word.lower(), 'limit': 1}, headers = {'app_id': id, 'app_key': key}).json()
    if data['results'] == []:
        return None
...
#        word = s.suggest(word)[0]
    ...
    word_id = search(word, id, key)
    if word_id is None:
        return 'I couldn\'t find any definitions for that word.'

    data = requests.get(url + '/entries/en/' + word_id + '/definitions', headers = {'app_id' : id, 'app_key' : key})
...
def dictionary(bot, trigger):
    ...
    msg = lookup(word, bot.config.dict.id, bot.config.dict.key)
    bot.say(msg)
    return
