...
@commands("d")
@priority("medium")
@example(".roll 3d1+1", 'You roll 3d1+1: (1+1+1)+1 = 4')
@example(".roll 3d1v2+1", 'You roll 3d1v2+1: (1[+1+1])+1 = 2')
@example(".roll 2d4", 'You roll 2d4: \(\d\+\d\) = \d', re=True)
@example(".roll 100d1", '[^:]*: \(100x1\) = 100', re=True)
@example(".roll 1001d1", 'I only have 1000 dice. =(')
@example(".roll 1d1 + 1d1", 'You roll 1d1 + 1d1: (1) + (1) = 2')
@example(".roll 1d1+1d1", 'You roll 1d1+1d1: (1)+(1) = 2')
...
def toss_command(bot, trigger):
    ...
    if random.randint(1,100) == 1:
        bot.say('It landed on its edge :o')
        return
