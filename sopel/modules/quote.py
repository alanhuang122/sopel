...
def quote(bot, trigger):
    ...
    if raw_args is None or raw_args.strip() == '':
        ...
        if len(command_parts) < 2:
            if validate_number_input(command_parts[0]) == '':
                output = dataprovider.get_by_id(int(command_parts[0]))
            else:
            ...
            # perform subcommand
            if subcommand == 'add':
                output = add_quote(dataprovider, data)
            elif subcommand == 'delete' or subcommand == 'remove':
                if trigger.nick in bot.config.core.admins or trigger.nick == bot.config.core.owner:
                    output = validate_number_input(data)
                    output = dataprovider.remove(int(data)) if output == '' else output
                else:
                    output = "I'm sorry {0}, I'm afraid I can't do that".format(trigger.nick)
...
class SqliteQuoteDataProvider(QuoteDataProvider):
    ...
    def add(self, line_to_add):
        ...
        msg = 'quote added: "{0}"'.format(line_to_add)
        return msg
